...
export default function StreamingServicesPage() {
  ...
  const handleSelectAllFree = () => {
    ...
    if (selectAllFree) {
      setSelectedServices((prev) => prev.filter((id) => !freeServiceIds.includes(id)));
      setSelectAllFree(false);
    } else {
      setSelectedServices((prev) => {
        const nonFreeServices = prev.filter((id) => !freeServiceIds.includes(id));
        return [...nonFreeServices, ...freeServiceIds];
      });
      setSelectAllFree(true);
    }
  };
